...
You can use Docker Compose to run the project with the provided `docker-compose.yml` file. This file defines a service named `epicgames-freegames-node` using the Docker image `ghcr.io/claabs/epicgames-freegames-node:latest`. The service mounts the `config` directory to `/usr/app/config` and exposes port 3000.
...
2. Create a `config` directory on your host machine and place your `config.json` file inside it.
3. Update the `docker-compose.yml` file with the appropriate host directory for the `config` volume.
...
This will start the `epicgames-freegames-node` service, and you can access the web server on port 3000.
